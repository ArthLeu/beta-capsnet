...
def load_batch_latent_from_file(file_list):
    ...
    for i in range(len(file_list)//batch_size):
        ...
        all_batch.append(latent_capsules)
...
def main():
    ...
    capsule_net = CapsuleBVAE(opt.latent_vec_size)
    ...
    print("[INFO] Loaded %d batches in total"%(len(all_batch)))
    ...
    for epoch in range(opt.n_epochs+1):
        ...
        for batch_id, latent_capsules in enumerate(all_batch):
            ...
            caps_recon, mu, logvar = capsule_net(latent_capsules)
            ...
            train_loss = recon_loss + beta_total_loss # LOSS (can be weighted)
